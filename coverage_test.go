...
func TestSupplementalReadLongFileAndRangeBranches(t *testing.T) {
	...
	out := coverageTestCaptureStdout(t, func() { _ = cmdRead(path) })
	...
	out = coverageTestCaptureStdout(t, func() { _ = cmdReadRange(path, 1, 0, "") })
	if !strings.Contains(out, "1#") || !strings.Contains(out, "2#") {
		t.Fatalf("cmdReadRange limit<=0 output unexpected: %q", out)
	}

	out = coverageTestCaptureStdout(t, func() { _ = cmdReadRange(filepath.Join(dir, "missing.txt"), 1, 1, "") })
	...
	out = coverageTestCaptureStdout(t, func() { _ = cmdReadRange(bin, 1, 1, "") })
